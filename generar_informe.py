...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .stat-card.pautas {{ border-left-color: #6f42c1; }}
        .stat-number {{ font-size: 2.5em; font-weight: bold; margin-bottom: 5px; }}
        .positive-text {{ color: #28a745; }}
        .negative-text {{ color: #dc3545; }}
        .neutral-text {{ color: #ffc107; }}
        .total-text {{ color: #007bff; }}
        .pautas-text {{ color: #6f42c1; }}
        ...
        .chart-container.extra-large {{ grid-column: 1 / -1; height: 600px; }}
        .comment-item {{ margin-bottom: 10px; padding: 15px; border-radius: 8px; border-left: 5px solid; word-wrap: break-word; }}
        .comment-positive {{ border-left-color: #28a745; background: #f0fff4; }}
        .comment-negative {{ border-left-color: #dc3545; background: #fff5f5; }}
        .comment-neutral {{ border-left-color: #ffc107; background: #fffbeb; }}
        ...
        @media (max-width: 900px) {{ 
            ...
            .chart-container.extra-large {{ height: 500px; }}
        }}
    </style>
</head>
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <div class="filters">
                ...
                <label for="postFilter">Pauta Específica:</label>
                <select id="postFilter">{post_filter_options}</select>
                <label for="topicFilter">Tema:</label>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            ...
            Object.assign(charts, {{
                ...
                sentimentByTopic: new Chart(document.getElementById('sentimentByTopicChart'), {{ 
                    type: 'bar',
                    options: {{ 
                        responsive: true,
                        maintainAspectRatio: false,
                        ...
                        plugins: {{ 
                            title: {{ display: true, text: 'Sentimiento por Tema' }},
                ...
                daily: new Chart(document.getElementById('dailyChart'), {{ 
                    ...
                    options: {{ 
                        ...
                        plugins: {{ 
                            title: {{ display: true, text: 'Volumen de Comentarios por Día' }},
                            datalabels: {{ display: false }}
                        }}
                    }}
                }}),
                hourly: new Chart(document.getElementById('hourlyChart'), {{ 
                    type: 'bar',
                    options: {{ 
                        responsive: true,
                        maintainAspectRatio: false,
                        scales: {{ 
                            ...
                            y: {{ 
                                stacked: true,
                                position: 'left',
                                title: {{ display: true, text: 'Comentarios por Hora' }}
                            }},
                            y1: {{ 
                                position: 'right',
                        ...
                        plugins: {{ 
                            title: {{ display: true, text: 'Volumen de Comentarios por Hora' }},
                            datalabels: {{ display: false }}
                        }}
                    }}
                }})
            ...
            const updatePostLinks = () => {{
                ...
                if (totalPages > 1) {{
                    ...
                    document.getElementById('prevPageBtn')?.addEventListener('click', () => {{
                        if (postLinksCurrentPage > 1) {{
                            postLinksCurrentPage--;
                            updatePostLinks();
                        }}
                    }});
                    document.getElementById('nextPageBtn')?.addEventListener('click', () => {{
                        if (postLinksCurrentPage < totalPages) {{
                            postLinksCurrentPage++;
                            updatePostLinks();
                        }}
                    }});
            ...
            const updateCharts = (postsData, filteredData) => {{
                ...
                charts.postCount.data.datasets = [{{
                    data: postCountLabels.map(p => postCounts[p]),
                    backgroundColor: ['#007bff', '#6f42c1', '#dc3545', '#ffc107', '#28a745']
                }}];
                charts.postCount.update();
                
                const sentimentCounts = filteredData.reduce((acc, curr) => {{
                    acc[curr.sentiment] = (acc[curr.sentiment] || 0) + 1;
                    return acc;
                }}, {{}});
                charts.sentiment.data.labels = ['Positivo', 'Negativo', 'Neutro'];
                charts.sentiment.data.datasets = [{{
                    data: [sentimentCounts['Positivo']||0, sentimentCounts['Negativo']||0, sentimentCounts['Neutro']||0],
                ...
                const topicColors = [
                    ...
                    '#34495e', '#95a5a6', '#e67e22', '#16a085', '#c0392b',
                    '#2ecc71', '#8e44ad', '#27ae60', '#d35400', '#2980b9',
                    '#c0392b', '#7f8c8d', '#f1c40f', '#e84393', '#00b894',
                    '#0984e3', '#6c5ce7', '#fdcb6e', '#d63031', '#00cec9'
                ];
                ...
                const sbtCounts = filteredData.reduce((acc, curr) => {{
                    if (!acc[curr.topic]) acc[curr.topic] = {{ Positivo: 0, Negativo: 0, Neutro: 0 }};
                    acc[curr.topic][curr.sentiment]++;
                    return acc;
                }}, {{}});
                const sbtLabels = Object.keys(sbtCounts).sort((a,b) => 
                    (sbtCounts[b].Positivo + sbtCounts[b].Negativo + sbtCounts[b].Neutro) - 
                    (sbtCounts[a].Positivo + sbtCounts[a].Negativo + sbtCounts[a].Neutro)
                );
                ...
                charts.daily.data.labels = sortedDays.map(d => 
                ...
                const hourlyCounts = filteredData.reduce((acc, curr) => {{
                    const hour = curr.date.substring(0, 13) + ':00:00';
                    if (!acc[hour]) acc[hour] = {{ Positivo: 0, Negativo: 0, Neutro: 0, Total: 0 }};
                    acc[hour][curr.sentiment]++;
                    acc[hour].Total++;
                    return acc;
                }}, {{}});
                const sortedHours = Object.keys(hourlyCounts).sort();
                let cumulative = 0;
                const cumulativeData = sortedHours.map(h => {{
                    cumulative += hourlyCounts[h].Total;
                    return cumulative;
                }});
                charts.hourly.data.labels = sortedHours.map(h => 
                    new Date(h).toLocaleString('es-CO', {{
                        day: '2-digit',
                        month: 'short',
                        hour: '2-digit',
                        minute: '2-digit'
            ...
            const updatePostFilterOptions = () => {{
                ...
                postsToShow.forEach(p => {{
                    postFilter.innerHTML += `<option value="${{p.post_url}}">${{p.post_label}}</option>`;
                }});
                if (postsToShow.some(p => p.post_url === currentPostSelection)) {{
